...
export default function CustomBuilder() {
  ...
  useEffect(() => {
    ...
    api.get('/layers').then(r => {
      ...
      const defaults = (r.data || []).slice(0, 2).map((lay, i) => ({
        ...
        density: lay.density,
        thicknessInch: 2,
        ratePerCubicCm: lay.ratePerCubicCm,
        orderSequence: i
      }));
      setLayers(defaults);
      setLoading(false);
  ...
  function updateLayer(index, field, value) {
    ...
    if (field === 'layerId') {
      ...
      if (lay) {
        updated[index] = { ...updated[index], layerId: lay._id, layerName: lay.name, materialType: lay.materialType, density: lay.density, ratePerCubicCm: lay.ratePerCubicCm };
      }
    } else {
      ...
      <div className="min-h-screen -m-4 bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-2xl p-8 text-center max-w-lg w-full">
          ...
          <div className="space-y-3">
            ...
            <Link to="/" className="block w-full px-6 py-3 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200">
              Back to Home
            </Link>
          </div>
        </div>
  ...
  if (showPayment) {
    return (
      <div className="min-h-screen -m-4 bg-gradient-to-br from-indigo-50 to-purple-100 flex items-center justify-center p-6">
  ...
  return (
    <div className="min-h-screen -m-4 bg-gray-50">
      {/* Header */}
      <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-indigo-800 text-white py-12 px-6">
        <div className="max-w-7xl mx-auto">
          <nav className="text-sm text-indigo-200 mb-4">
            <Link to="/" className="hover:text-white">Home</Link> <span className="mx-2">/</span> Custom Builder
          </nav>
          <h1 className="text-4xl font-bold mb-2">Custom Mattress Builder</h1>
          <p className="text-indigo-200">Design your perfect mattress layer by layer with live pricing</p>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="number" value={length} onChange={e => setLength(Number(e.target.value))} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-lg" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Width (inches)</label>
                  <input type="number" value={width} onChange={e => setWidth(Number(e.target.value))} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-lg" />
                </div>
              </div>
              <div className="mt-4 flex gap-4 flex-wrap">
                {[{l:75,w:36,n:'Single (75x36)'},{l:75,w:48,n:'Double (75x48)'},{l:78,w:60,n:'Queen (78x60)'},{l:78,w:72,n:'King (78x72)'}].map(s => (
                  <button key={s.n} onClick={() => {setLength(s.l);setWidth(s.w);}} className={'px-4 py-2 rounded-lg text-sm font-medium transition-all ' + (length===s.l && width===s.w ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>
                    {s.n}
                  </button>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={addLayer} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors">
                  + Add Layer
                </button>
              </div>
                      ...
                      <div key={idx} className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-4 border border-gray-200">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            ...
                            <select value={layer.layerId} onChange={e => updateLayer(idx, 'layerId', e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                          ...
                          <div>
                            ...
                            <input type="number" value={layer.thicknessInch} onChange={e => updateLayer(idx, 'thicknessInch', Number(e.target.value))} min="0.5" max="8" step="0.5" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500" />
                          </div>
                          <div>
                        ...
                        <div className="mt-2 text-xs text-gray-400">
                          {layer.materialType} | Density: {layer.density} kg/m3 | Rate: Rs.{layer.ratePerCubicCm}/cm3
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={customerInfo.name} onChange={e => setCustomerInfo({...customerInfo, name: e.target.value})} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500" placeholder="Enter your name" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Phone Number *</label>
                  <input type="tel" value={customerInfo.phone} onChange={e => setCustomerInfo({...customerInfo, phone: e.target.value})} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500" placeholder="Enter phone number" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-2">Delivery Address</label>
                  <textarea value={customerInfo.address} onChange={e => setCustomerInfo({...customerInfo, address: e.target.value})} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500" rows="2" placeholder="Enter delivery address"></textarea>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 sticky top-6">
              ...
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-4 mb-6">
                <div className="text-center">
                  <svg className="w-16 h-16 mx-auto text-indigo-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="mt-4 space-y-1">
                  {layers.map((l, i) => (
                    <div key={i} className="h-4 rounded bg-gradient-to-r from-indigo-400 to-purple-400 relative" style={{opacity: 0.5 + (i * 0.15)}}>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="border-t pt-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-2xl font-bold text-indigo-600">Rs.{total.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl hover:shadow-lg hover:shadow-indigo-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Proceed to Payment
              </button>
